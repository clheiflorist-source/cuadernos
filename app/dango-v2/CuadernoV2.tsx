...
import { TEXTOS_UI, type Lang, type TextosUI } from "./contenido";

/**
 * Edición v2 · experimento premium bilingüe (solo DÁNGO, en evaluación).
 *
 * Misma partitura que components/CuadernoUI — páginas, autoguardado,
 * continuidad, índice, swipe — con dos capas nuevas:
 *
 * 1. ENCUADERNACIÓN NOCTURNA. La portada y la página final se visten de
 *    carbón espresso con marco de filete doble y título en oro estampado
 *    (foil): el mismo libro de páginas crema, pero en su edición de lujo.
 *    El interior conserva la crema — la legibilidad manda — con numerales
 *    fantasma en las aperturas, capitulares en las cartas y un hilo de
 *    avance con destello champaña.
 *
 * 2. ESPAÑOL ⇄ INGLÉS. Un conmutador en la cabecera. Las respuestas viven
 *    aparte del idioma (mismos ids de campo), así que cambiar de lengua
 *    nunca borra nada. La preferencia persiste con el borrador y puede
 *    fijarse por URL con ?lang=en.
 *
 * Si el experimento no convence, esta carpeta se elimina y los cuatro
 * cuadernos publicados quedan intactos.
 */
...
/** Renglón para los nombres. En la portada nocturna escribe en marfil. */
  ...
  return (
    <div className={`relative ${className}`}>
      <input
        ...
        className={`cuaderno-writing peer w-full border-0 border-b bg-transparent pb-2 text-center font-editorial text-2xl italic outline-none placeholder:transition-opacity placeholder:duration-500 focus:placeholder:opacity-40 ${
          oscuro
            ? "border-ivory/25 text-ivory placeholder:text-ivory/35"
            : "border-line text-charcoal placeholder:text-ink-faint/60"
        }`}
      />
      <span
        aria-hidden="true"
        className={`pointer-events-none absolute inset-x-0 bottom-0 h-px origin-center scale-x-0 transition-transform duration-500 ease-out peer-focus:scale-x-100 ${
          oscuro ? "bg-champagne" : "bg-gold-deep"
        }`}
      />
    </div>
  );
...
function CampoBloque({
  ...
  return (
    <div>
      <p className="font-display text-sm tracking-[0.24em] text-gold-deep">
        {numero}.
      </p>
...
export default function CuadernoV2({
  contenidos,
  acciones,
  continuidadDisponible,
}: {
  contenidos: { es: ContenidoCuaderno; en: ContenidoCuaderno };
  acciones: AccionesCuaderno;
  continuidadDisponible: boolean;
}) {
  const [lang, setLang] = useState<Lang>("es");
  const C = contenidos[lang];
  ...
  // Modelo de páginas: misma estructura en ambos idiomas (mismos ids y
  // mismo número de páginas), así que el folio sobrevive al conmutador.
  ...
  const LS_KEY = `clhei-cuaderno-${contenidos.es.libro.slug}-v1`;
  ...
  useEffect(() => {
    (async () => {
      ...
      if (langUrl === "en" || langUrl === "es") setLang(langUrl);
      else if (locales?.lang === "en" || locales?.lang === "es")
  ...
  const capActual =
    actual.tipo === "apertura" || actual.tipo === "preguntas"
      ? actual.cap
      : null;
  /* La encuadernación: portada y página final se visten de noche. */
  const oscuro = actual.tipo === "portada" || actual.tipo === "final";
  ...
  if (!listo) return <div className="min-h-dvh bg-night" />;

  return (
    <div
      className={`relative min-h-dvh transition-colors duration-700 ${
        oscuro ? "bg-night" : "bg-cream"
      }`}
    >
      {/* ——— Marca de agua viva + anillo estela ——— */}
      <Ambiente pagina={pagina} />

      {/* ——— Marco de filete doble: solo sobre la encuadernación ——— */}
      {oscuro && (
        <div aria-hidden="true" className="pointer-events-none fixed inset-0 z-[5]">
          <div className="absolute inset-3 border border-champagne/25 md:inset-5" />
          <div className="absolute inset-[19px] border border-champagne/10 md:inset-8" />
        </div>
      )}
      ...
      <header
        className={`fixed inset-x-0 top-0 z-30 ${
          oscuro ? "" : "bg-cream/85 backdrop-blur-sm"
        }`}
      >
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-5 md:h-20 md:px-8">
          <span className="flex items-center gap-3 md:gap-4">
            <span
              className={`font-display text-[0.85rem] tracking-[0.3em] md:text-[0.95rem] ${
                oscuro ? "text-ivory" : "text-charcoal"
              }`}
            >
              CLHEI
            </span>
            <span
              ...
              className={`hidden h-3.5 w-px sm:block ${
                oscuro ? "bg-champagne/40" : "bg-line-gold"
              }`}
            />
            <span
              className={`hidden font-display text-[0.7rem] uppercase tracking-[0.3em] sm:block md:text-[0.75rem] ${
                oscuro ? "text-champagne" : "text-gold-deep"
              }`}
            >
              {LIBRO.etiqueta}
            </span>
          ...
          <span className="flex items-center gap-4 md:gap-7">
            ...
            <span
              aria-live="polite"
              className={`kicker transition-opacity duration-700 ${
                oscuro ? "text-champagne/90" : "text-sage"
                ...
                <span key={l} className="flex items-center gap-2">
                  {i > 0 && (
                    <span
                      aria-hidden="true"
                      className={`h-3 w-px ${
                        oscuro ? "bg-ivory/25" : "bg-line-gold"
                      }`}
                    />
                  )}
                  <button
                    ...
                    onClick={() => setLang(l)}
                    aria-pressed={lang === l}
                    className={`cursor-pointer font-display text-[0.68rem] tracking-[0.28em] uppercase transition-colors duration-300 ${
                      lang === l
                        ? oscuro
                          ? "text-champagne underline decoration-champagne/50 underline-offset-4"
        ...
        <div className={oscuro ? "h-px w-full bg-ivory/10" : "h-px w-full bg-line/60"}>
          <div
            className="v2-thread h-px w-full origin-left transition-transform duration-700"
      ...
      <main className="relative z-10 mx-auto w-full max-w-2xl px-6 pb-40 pt-28 md:px-8 md:pt-32">
        <AnimatePresence mode="wait">
          ...
          >
            {/* Portada: la encuadernación nocturna con oro estampado */}
            {actual.tipo === "portada" && (
              <div className="flex min-h-[70dvh] flex-col items-center justify-center text-center">
                ...
                />
                <p className="kicker mt-10 !text-champagne">
                  {LIBRO.subtitulo}
                </p>
                <h1 className="v2-foil font-display mt-6 text-4xl leading-tight tracking-[0.08em] md:text-5xl">
                  {titulo1}
                  {titulo2 && (
                    <>
                      <br />
                ...
                <Ornamento className="mt-8 w-28 text-champagne/60" />
                <p className="kicker mt-8 !text-ivory/70">{LIBRO.servicio}</p>
                <LineaNombres
                  valor={nombres}
                  onCambio={setNombres}
                ...
                <button
                  type="button"
                  onClick={() => irA(1)}
                  className="kicker mt-14 cursor-pointer border border-champagne/50 px-9 py-3.5 !text-ivory transition-colors duration-300 hover:border-champagne hover:bg-champagne hover:!text-night"
                >
                  {T.abrir}
                </button>
                <p className="mt-16 text-[0.7rem] tracking-[0.2em] text-ivory/35">
            ...
            {/* Apertura de capítulo — con numeral fantasma */}
            ...
            {actual.tipo === "entrega" && (
              <div className="flex min-h-[64dvh] flex-col justify-center text-center">
                ...
                <button
                  ...
                  className="kicker mx-auto mt-12 cursor-pointer border border-gold-deep/50 px-10 py-4 text-charcoal transition-colors duration-300 hover:border-gold-deep hover:bg-gold-deep hover:!text-cream disabled:cursor-default disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:!text-charcoal"
                >
            ...
            {/* Página final: el otro lado de la encuadernación */}
            {actual.tipo === "final" && (
              <div className="flex min-h-[70dvh] flex-col items-center justify-center text-center">
                <LogoMark
                  size={50}
                ...
                />
                <h1 className="font-display mt-10 text-3xl tracking-[0.06em] text-ivory">
                  {T.finalTitulo}
                </h1>
                <p className="mx-auto mt-6 max-w-md leading-[1.9] text-ivory/70">
                  {T.finalGracias(nombres.trim())}
                </p>
                <Ornamento className="mx-auto mt-12 w-28 text-champagne/50" />
                <p className="accent-italic mx-auto mt-10 max-w-md text-lg leading-[1.8] text-champagne">
                  {FRASE_FINAL}
                </p>
                <p className="mt-16 text-[0.7rem] tracking-[0.2em] text-ivory/35">
                  {LIBRO.marca.toUpperCase()}
